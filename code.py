...
from adafruit_httpserver import Server, Request, Response, ServerStoppedError
...
status_label = label.Label(terminalio.FONT, text="Status: ---", color=0xFFFFFF, x=0, y=30)
...
def mount_sd(intermediate):
    try:
        ...
        print("SD gemountet")
        intermediate.status = "SD gemountet"
        return True
    except Exception as e:
        print("SD Fehler:", e)
        intermediate.status = "SD Fehler"
        return False
...
async def task_sd_writer(intermediate):
    while True:
        if sd_log_queue and intermediate.logging_enabled: # new logging_enabled logic
            csv_line, json_line = sd_log_queue.pop(0)
            try:
                ...
                with open(f"{SD_PATH}/{JSON_FILE}", "a") as f:
                    f.write(json_line)
                    f.flush()
                # print("SD Log geschrieben")
            except Exception as e:
                print("SD Log Fehler:", e)
    ...
    while True:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                intermediate.status = f"Shelly Fehler {r.status_code}"
                await set_status_led(COLOR_ERR)

        except OSError as e:
            intermediate.status = "Shelly offline"
            await set_status_led(COLOR_ERR)
...
def start_server():
    ...
    if intermediate.ap_ip:
        server.start(str(intermediate.ap_ip), 80)
...
def index(request: Request):
    try:
        ...
        html = "<h1>Fehler: HTML-Datei nicht gefunden!</h1>"
    finally:
        gc.collect()
...
def static_file(request: Request, filename):
    ...
    try:
        ...
        return Response(request, data, content_type=mime)

    except Exception as e:
        #print("❌ STATIC ERROR:", e)
        return Response(request, "404 Not Found", status=404)
...
def status_json(request: Request):
    ...
    data = {
        ...
        "ap_ip": intermediate.ap_ip or "-",
        "sta_ip": intermediate.sta_ip or "-",
...
async def task_display():
    while True:
        ...
        status_label.text = intermediate.status
